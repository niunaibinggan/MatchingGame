...
export default class ResultPanel extends Hilo.Container {
  ...
  submitButton(properties) {
    properties.subBtn.on(Hilo.event.POINTER_START, e => {
      ...
      this.setAnswer.forEach((item, index) => {
        if (item && item.questionId !== this.rightQuestions[index].id) {
          Hilo.Tween.to(
            that.leftContainer.getChildAt(item.realId),
            {
              x: that.selectPosition[item.realId].x,
              y: that.selectPosition[item.realId].y
            },
            { duration: 300 }
          );
      ...
      this.answerContainer.children.forEach(item => {
        Hilo.Tween.to(
          ...
          { alpha: 0.3 },
          {
            ...
            onComplete() {
              item.alpha = 0.3;
              Hilo.Tween.to(item, { alpha: 0 }, { duration: 800 });
            }
          }
        );
      });
  ...
  commonPanel(data) {
    ...
    target.forEach((item, index) => {
      ...
      new Text({
        ...
        text: item.text,
        fontSize:
          getByte(item.text) < 10
            ? 40
            : 40 - Math.round(getByte(item.text) / 8) * 5,
        bold: true,
        textAlign: "center",
        visible: true,
        alpha: 1,
        ...
        height: rect[3] - 10,
        x: type === "left" ? 0 : this.rectLeft[2] - 12,
        y: Math.floor(getByte(item.text) / 4)
          ? 10 + (40 - (40 - Math.round(getByte(item.text) / 6) * 5)) / 2
          : 10,
        color: "#fff"
      }).addTo(questionsItem);
  ...
  drag(target) {
    ...
    target.on("dragEnd", e => {
      ...
      const existIndex = this.setAnswer.findIndex(
        item => item && item.realId === targetIndex
      );
      ...
      const x = isSelected
        ? this.targetPosition[currentTarget].x
        : this.selectPosition[targetIndex].x;

      const y = isSelected
        ? this.targetPosition[currentTarget].y
        : this.selectPosition[targetIndex].y;
      ...
      if (isSelected && this.setAnswer[currentTarget]) {
        ...
        Hilo.Tween.to(
          ...
          {
            x: this.selectPosition[preIndex].x,
            y: this.selectPosition[preIndex].y
          },
          { duration: 200 }
        );
      }
  ...
  findItemIndex(dragX, dragY, type) {
    ...
    const distanceContainerY = Math.abs(
      this.leftContainerY - this.rightContainerY
    );
    ...
    const filterArr = this.targetPosition.filter((item, index) => {
      if (
        item.x + this.rectLeft[2] > dragX &&
        item.x - this.rectLeft[2] < dragX &&
        item.y + this.rectLeft[3] > dragY &&
        item.y - this.rectLeft[3] < dragY
      ) {
        item.index = index;
    ...
    return filterArr[
      filterArr.findIndex(item => item.distanceY === minDistance)
    ].index;
  }
}
